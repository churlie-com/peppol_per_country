...
### 2. implement the different actions in Script:main() directly or with helper functions download_huge_xml
### 3. implement helper functions you defined in previous step
### ==============================================================================

### Created by Peter Forret ( pforret ) on 2025-11-03
...
readonly script_created="2025-11-03"
readonly run_as_root=-1 # run_as_root: 0 = don't check anything / 1 = script MUST run as root / -1 = script MAY NOT run as root
readonly script_description="Synchronize PEPPOL export into git-managed files"
...
flag|p|push|commit and push changes to git
option|L|LOG_DIR|folder for log files |log
option|T|TMP_DIR|folder for temp files|tmp
#option|W|WIDTH|width of the picture|800
choice|1|action|action to perform|sync,check,env,update
...
function Script:main() {
  ...
  case "${action,,}" in
  sync)
    #TIP: use «$script_prefix sync» to ...
    #TIP:> $script_prefix sync
    download_huge_xml
    split_into_extracts
    commit_and_push
    ;;
...
function download_huge_xml() {
  Os:require "curl"

  local url="https://directory.peppol.eu/export/businesscards"
  local output_file="$TMP_DIR/directory-export-business-cards.xml"

  [[ -f $output_file ]] && return 0

  IO:announce "Downloading PEPPOL export from $url"
  IO:log "download_huge_xml: $url"
  ...
  if curl -f -L -o "$output_file" "$url"; then
    ...
    if [[ -f "$output_file" ]]; then
      ...
      IO:debug "File size: $file_size"
      IO:log "download_huge_xml: $file_size downloaded"
    fi
  else
    IO:die "Failed to download from $url"
  fi
}

function split_into_extracts() {
  Os:require "xmllint"

  local input_file="$TMP_DIR/directory-export-business-cards.xml"

  [[ ! -f "$input_file" ]] && IO:die "Input file not found: $input_file"

  IO:announce "Splitting XML into country/month extracts"
  IO:log "split_into_extracts: Starting extraction"

  # Create extracts directory
  mkdir -p extracts

  # Get all unique country codes - fast grep without loading file into memory
  local countries_cache="$TMP_DIR/countries.txt"
  local countries

  if [[ -f "$countries_cache" ]]; then
    IO:progress "Loading cached country codes..."
    countries=$(cat "$countries_cache")
    IO:debug "Loaded country codes from cache"
  else
    IO:progress "Extracting country codes..."
    countries=$(grep -E '<entity countrycode="\w+">' "$input_file" | cut -d '"' -f2 | sort -u)

    if [[ -z "$countries" ]]; then
      IO:die "No country data found in XML file"
    fi

    # Save to cache for next time
    echo "$countries" > "$countries_cache"
    IO:debug "Saved country codes to cache"
  fi

  local country_count
  country_count=$(echo "$countries" | wc -l | xargs)
  IO:debug "Found $country_count countries: $countries"
  IO:log "split_into_extracts: Found $country_count countries"

  # Process each country by streaming through the file
  local processed=0
  for country in $countries; do
    ((processed++))
    IO:progress "Processing country $country ($processed/$country_count)..."
    IO:log "split_into_extracts: Processing country $country ($processed/$country_count)"

    # Create temp file for this country's business cards
    local temp_cards="$TMP_DIR/cards.$country.xml"

    # Stream extract business cards for this country using awk (memory efficient)
    awk -v country="$country" '
      ...
      in_card {
        card = card $0 "\n"
        if (/<entity countrycode="/ && index($0, "countrycode=\"" country "\"")) {
          match_country = 1
        }
        if (/<regdate>/) {
          # Extract date value using sub/gsub (BSD awk compatible)
          line = $0
          gsub(/^[^>]*>/, "", line)
          gsub(/<.*$/, "", line)
          regdate = line
    ...
    ' "$input_file" > "$temp_cards"

    # Check if we found any cards for this country
    if [[ ! -s "$temp_cards" ]]; then
      rm -f "$temp_cards"
      continue
    fi

    # Get unique year-months from this country's data
    local months
    months=$(cut -d'|' -f3 "$temp_cards" | grep -E '^[0-9]{4}-[0-9]{2}' | cut -c1-7 | sort -u)

    # Check if there are cards without dates and add 2000-00 to months list
    if grep -q '||$' "$temp_cards"; then
      months="$months"$'\n'"2000-00"
    fi

    # If no dates found at all, use 2000-00
    [[ -z "$months" ]] && months="2000-00"

    # Process each month
    for month in $months; do
      local month_folder="extracts/$month"
      mkdir -p "$month_folder"

      local output_file="$month_folder/businesscards.$country.xml"

      # Build the output XML file
      {
        cat "$script_install_folder/xml/header.xml.part"

        # Extract cards for this month
        awk -v month="$month" '
          {
            # Split on || delimiter manually (BSD awk compatible)
            split_pos = index($0, "||")
            if (split_pos > 0) {
              card = substr($0, 1, split_pos - 1)
              date = substr($0, split_pos + 2)
            } else {
              card = $0
              date = ""
            }
            # For month 2000-00, only include cards without dates
            # For other months, only include cards matching that month
            if ((month == "2000-00" && date == "") || substr(date, 1, 7) == month) {
              # Add indentation to card
              gsub(/\n/, "\n    ", card)
              print "    " card
            }
          }
        ...
        cat "$script_install_folder/xml/footer.xml.part"
      } > "$output_file"

      IO:log "split_into_extracts: Created $output_file"

      # Pretty-print the XML if possible
      if xmllint --format "$output_file" > "$output_file.tmp" 2>/dev/null; then
        mv "$output_file.tmp" "$output_file"
      else
        rm -f "$output_file.tmp"
      fi

      IO:debug "Created: $output_file"
    done

    # Clean up temp file for this country
    rm -f "$temp_cards"
  done

  IO:success "Split complete - extracts saved to extracts/"
  IO:log "split_into_extracts: Completed successfully"
}
...
function commit_and_push(){
  ...
  if git diff --quiet --cached; then
    ...
    if [[ -z "${RUNNER_OS:-}" ]] ; then
      ...
      if ((push)); then
        # --push flag specified, do git commit and push
        local timestamp message
        timestamp="$(date -u)"
        message="$timestamp < $script_basename $script_version"
        IO:print "Commit Message: $message"
        git commit -m "${message}" || IO:die "Git commit failed"
        IO:print "Pulling with rebase..."
        git pull --rebase || IO:die "Git pull --rebase failed"
        IO:print "Pushing changes..."
        git push || IO:die "Git push failed"
        IO:success "Commit and push OK!"
      else
        # no --push flag, just update version
        setver auto
      fi
    else
      # in GitHub action
